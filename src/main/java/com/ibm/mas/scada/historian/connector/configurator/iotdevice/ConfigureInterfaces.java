...
public class  ConfigureInterfaces {
    ...
    public ConfigureInterfaces(int scadaType, String dataDir, String orgId, String key, String token) {
        String url = "";
        this.iotp = new IoTClient(url, orgId, key, token);
        this.dataDir = dataDir;
        this.scadaType = scadaType;
    }

    public ConfigureInterfaces(int scadaType, String baseUrl, String dataDir, String orgId, String key, String token) {
        this.iotp = new IoTClient(baseUrl, orgId, key, token);
        this.dataDir = dataDir;
        this.scadaType = scadaType;
    }
    ...
    public void config(String deviceType) throws Exception {
        ...
        try {
            ...
            if (li == null) {
                JSONObject piSchema = CreateSchemas.buildSchema(scadaType, "eventSchema");
                ...
                JSONObject liSchema = CreateSchemas.buildSchema(scadaType, "liSchema");
                ...
                if (scadaType == Constants.SCADA_OSIPI) {
                    ...
                    mapping.put("intvalue", "$exists($event.intvalue) ? $event.intvalue: null");
                    mapping.put("floatvalue", "$exists($event.floatvalue) ? $event.floatvalue: null");
                    mapping.put("stringvalue", "$exists($event.stringvalue) ? $event.stringvalue: null");
                    mapping.put("datevalue", "$exists($event.datevalue) ? $event.datevalue: null");
                    mapping.put("type", "$exists($event.type) ? $event.type: null");
                    mapping.put("decimalAccuracy", "$exists($event.decimalAccuracy) ? $event.decimalAccuracy: null");
                    mapping.put("evt_timestamp", "$exists($event.evt_timestamp) ? $event.evt_timestamp: null");
                    mapping.put("unit", "$exists($event.unit) ? $event.unit: null");
